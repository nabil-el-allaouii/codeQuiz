let homeQuiz = document.querySelector(".section");
async function homemake() {
    const response = await fetch("http://localhost:3000/quizes/");
    const data = await response.json();
    data.forEach((e, m) => {
        if (data[m].status === "actif") {
            let template =
                `<div class="quiz">
        <div>
        ...
        <a href="quiz.html" id="${e.id}">
            ...
            homeQuiz.innerHTML += template;
        }

        let quizesbutton = document.querySelectorAll(".quiz-button");
        quizesbutton.forEach((k) => {
            k.addEventListener("click", () => {
                localStorage.setItem("id", k.parentElement.id);
            })
        })
    })
}
